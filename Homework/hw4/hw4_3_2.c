...
//  int* loop_buffer;             /* A buffer to store all commands within a loop. */
//  size_t loop_pointer_offset;
...
struct brainfuck_state* brainfuck_state_new() {
  struct brainfuck_state* state = (struct brainfuck_state*)malloc(100*sizeof(struct brainfuck_state));
  state->memory_buffer = (uint8_t*)calloc(BRAINFUCK_MEMORY_BUFFER_SIZE,sizeof(uint8_t));
...
void brainfuck_execute_plus(struct brainfuck_state* state) {
  state->memory_buffer[state->memory_pointer_offset] = (state->memory_buffer[state->memory_pointer_offset]+1)%256;
}
...
void brainfuck_execute_minus(struct brainfuck_state* state) {
  ...
  {
    state->memory_buffer[state->memory_pointer_offset] = 255;
  }
  else
  {
    state->memory_buffer[state->memory_pointer_offset] = state->memory_buffer[state->memory_pointer_offset]-1;
  }
}
...
void brainfuck_execute_previous(struct brainfuck_state* state) {
  ...
  {
    state->memory_pointer_offset = state->memory_pointer_offset - 1;
  }
}
...
void brainfuck_execute_next(struct brainfuck_state* state) {
  state->memory_pointer_offset = (state->memory_pointer_offset + 1)%(BRAINFUCK_MEMORY_BUFFER_SIZE);
}
...
// ↓↓↓ FOR PART B ONLY ↓↓↓
void brainfuck_execute_loop_start(struct brainfuck_state* state, char* src, long *num){
  if (state->memory_buffer[state->memory_pointer_offset] == 0)
  {
    long loop_count = 0;
    while (loop_count != -1)
    {
      *num += 1;
      if (src[*num] == BRAINFUCK_TOKEN_LOOP_END)
      {
        loop_count -= 1;
      }
      else if (src[*num] == BRAINFUCK_TOKEN_LOOP_START)
      {
        loop_count += 1;
      }
    }
...
void brainfuck_execute_loop_end(struct brainfuck_state* state, char* src, long *num){
  ...
  if (state->memory_buffer[state->memory_pointer_offset] != 0)
  {
    ...
    {
      *num -= 1;
      if (src[*num] == BRAINFUCK_TOKEN_LOOP_START)
      {
        loop_count -= 1;
      }
      else if (src[*num] == BRAINFUCK_TOKEN_LOOP_END)
      {
        loop_count += 1;
      }
    }
...
bool brainfuck_main(struct brainfuck_state* state, char* src,
  ...
  for (long *num = &i; *num<len; (*num)++)
  {
    switch(src[*num])
    {
      ...
      brainfuck_execute_loop_start(state, src, num);
      break;
      case BRAINFUCK_TOKEN_LOOP_END:
      brainfuck_execute_loop_end(state, src, num);
      break;
    }
  }
  return true;
