...
public:
  ...
  // TODO: IMPLEMENT THIS.
  template <bool C> bool operator==(const SlistIterator<T, C> &other) const {return this->m_node == other.m_node;}
};
...
public:
  ...
  {
    m_head = std::make_unique<SlistNode<T>>(std::move(m_head), std::forward<Types>(args)...);
    return m_head->value;
  }

  // TODO: IMPLEMENT THIS.
  void pop_front(){m_head = std::move(m_head);}
  ...
  // TODO: IMPLEMENT THIS.
  void insert_after(const_iterator pos, T &&x) {emplace_after(pos, std::move(x));}

  // TODO: IMPLEMENT THIS.
  template <typename... Types>
  iterator emplace_after(const_iterator pos, Types &&...args)
  {
    pos.m_node->next = std::make_unique<SlistNode<T>>(std::move(pos.m_node->next), std::forward<Types>(args)...);
  ...
  {
    pos.m_node->next = std::move(pos.m_node->next->next);
    return pos.m_node.next;
  }
  ...
  {
    pos.m_node->next = std::move(last.m_node);
